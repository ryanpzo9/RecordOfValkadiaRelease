...
/*:
 * @plugindesc (v1.4) Animated title screen with characters and rotating effects.
 * @author Moghunter
 *
 * @param Char Number
 * @desc Amount of character images to display.
 * @default 4
 *
 * @param Char X-Axis
 * @desc X-axis offset for the character images.
 * @default 0
 *
 * @param Char Y-Axis
 * @desc Y-axis offset for the character images.
 * @default 0
 *
 * @param Flower X-Axis
 * @desc X-axis offset for the flower images.
 * @default 0
 *
 * @param Flower Y-Axis
 * @desc Y-axis offset for the flower images.
 * @default 0
 *
 * @param Rotation
 * @desc Enable the rotation effect.
 * @default true
 *
 * @param Rotation Time
 * @desc Duration (speed) of the rotation cycle.
 * @default 300
 *
 * @param Back 1 Scroll
 * @desc Enable scrolling movement for Background 1.
 * @default true
 *
 * @param Back 1 Tiled
 * @desc If 'true', tile/repeat the image. If 'false', stretch the image to fill the entire screen.
 * @default false
 *
 * @help  
 * =============================================================================
 * +++ MOG - Title Screen Himawari (v1.4) +++
 * By Moghunter 
 * https://atelierrgss.wordpress.com/
 * =============================================================================
 * Animated title scene with characters.
 * =============================================================================
 * Character images must be saved in the folder: /img/titles1/
 * * The naming of the character images must follow this format:
 *
 * Char_ + ID.png
 *
 * Char_1.png
 ...
 * ...
 *
 * It is advisable to use character images that match the screen height.
 * (e.g., 624 pixels) 
 *
 */
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_TitleHimawari');
    ...
    // NEW PARAMETERS
    Moghunter.himawari_back1_scroll = String(Moghunter.parameters['Back 1 Scroll'] || "true");
    Moghunter.himawari_back1_tiled = String(Moghunter.parameters['Back 1 Tiled'] || "false");
...
Scene_Title.prototype.createHbackground = function() {
    var isTiled = String(Moghunter.himawari_back1_tiled) === "true";
    
    // Set field dimensions
	var width = isTiled ? Graphics.boxWidth * 2 : Graphics.boxWidth;
	var height = isTiled ? Graphics.boxHeight * 2 : Graphics.boxHeight;
	
	this._background = [];

    // --- Background 1 Setup ---
    if (!isTiled) {
        // Full Screen Mode (Sprite)
        this._background[0] = new Sprite(this._back_img[0]);
        this._background[0].anchor.x = 0.5;
        this._background[0].anchor.y = 0.5;
        
        // Safety: If image is not loaded yet, width is 0. 
        // We set scale to 0 initially to avoid "Infinity" (black screen) glitches.
        // The scale will be corrected in 'firstRefresh' when the image is Ready.
        var imgW = this._back_img[0].width;
        var imgH = this._back_img[0].height;
        
        if (imgW > 0 && imgH > 0) {
            this._background[0].scale.x = width / imgW;
            this._background[0].scale.y = height / imgH;
        } else {
             this._background[0].scale.x = 0;
             this._background[0].scale.y = 0;
        }
        
        this._scenarioField.addChild(this._background[0]);
        
    } else {
        // Tiled Mode (TilingSprite)
        this._background[0] = new TilingSprite(this._back_img[0]);
        this._background[0].move(0,0, width, height);
        this._background[0].anchor.x = 0.5;
        this._background[0].anchor.y = 0.5;
        this._scenarioField.addChild(this._background[0]);
    }
    ...
    // --- Background 2 Setup ---
    var width2 = Graphics.boxWidth * 2;
    var height2 = Graphics.boxHeight * 2;
    this._background[1] = new TilingSprite(this._back_img[1]);
    this._background[1].move(0,0, width2, height2);
    this._background[1].anchor.x = 0.5;
    this._background[1].anchor.y = 0.5;
    this._scenarioField.addChild(this._background[1]);
...
Scene_Title.prototype.firstRefresh = function() {
     ...
     // --- FIX: Recalculate Scale for Non-Tiled Background ---
     if (String(Moghunter.himawari_back1_tiled) === "false" && this._background[0]) {
         var width = Graphics.boxWidth;
         var height = Graphics.boxHeight;
         var imgW = this._back_img[0].width;
         var imgH = this._back_img[0].height;
         // Only apply scale if the image is actually valid
         if (imgW > 0 && imgH > 0) {
            this._background[0].scale.x = width / imgW;
            this._background[0].scale.y = height / imgH;
         } else {
             // Fallback default just in case, though isReady check should prevent this
             this._background[0].scale.x = 1.0;
             this._background[0].scale.y = 1.0;
         }
     }
...
//==============================
// * Update Himawari (FIXED)
//==============================
Scene_Title.prototype.update_himawari = function() {
    // --- FIX: Wait for both CHARS and BACKGROUND before starting the scene ---
    // If we don't wait for Background 1, the scale calculation might fail and cause a black screen.
	if (this._firstRefresh && this._char_img[0].isReady() && this._back_img[0].isReady()) {
        this.firstRefresh();
    }
...
Scene_Title.prototype.update_background_effects = function() {
    // Check if scrolling is enabled AND if it's a TilingSprite (Fit mode cannot scroll infinitely)
    if (String(Moghunter.himawari_back1_scroll) === "true" && this._background[0] instanceof TilingSprite) {
