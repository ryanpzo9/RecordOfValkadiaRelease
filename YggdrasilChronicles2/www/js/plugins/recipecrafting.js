//==
// Recipe Crafting MV Version 1.3o
//==

/*:
 * @plugindesc Take some items.. and make them into other items! That's crafting~
 * @author Vlue (with additions from DragonPC, MuteDay, and JR)
 *
 * @param Categories
 ...
 * @desc Change Crafting Exp gained text. (default - Crafting Exp Gained:)
 ...
 *
 * @param DroppedBy Text
 * @desc Change Dropped By text for materials. (default - Dropped By:)
 * @default Dropped By:
 *
 * @param SoldIn Text
 * @desc Change Sold In text for materials. (default - Sold In:)
 * @default Sold In:
 *
 * @param DroppedBy Text Size
 * @desc Font size for Dropped By & Sold In text as a percentage of default font size (default - 75)
 * @default 75
 *
 * @param Crafting Text
 ...
 *  $gameParty.learnRecipe(value) (Learn recipe)
 *
 * Recipe Setup
 ...
 *
 * Modifications:
 * - Added "Dropped By" feature: Displays "[DroppedBy Text]: [Enemy Name(s)]" below each required material in the crafting UI.
 *   The plugin scans the Enemies database for note tags like <Item X: [Number]%>, <Armor Y: [Number]%>, or <Weapon Z: [Number]%> that match the required material.
 *   For example, if a material is Item ID 353 ("Voiceless Iron") and an enemy has the note tag <Item 353: 50%>, the UI will display "[DroppedBy Text]: Slime" below that material (drop percentage is not shown).
 *   If multiple enemies drop the same material, their names are listed as a comma-separated list (e.g., "[DroppedBy Text]: Slime, Goblin").
 *   The list of enemy names is constrained to show 3–4 enemies on the first line (with the "Dropped By" label), depending on available space, and the remaining enemies are moved to a second line below the "Dropped By" label.
 *   The "Dropped By" line is preceded by a small icon (icon index 92), indented further to the right (48 pixels), and displayed at a configurable font size (default 75% of the material name's font size, adjustable via "DroppedBy Text Size" parameter).
 *   The icon is scaled to 50% of its original size (16x16 pixels) to match the smaller "Dropped By" text size.
 *   The "Dropped By" text can be configured via the "DroppedBy Text" plugin parameter.
 *   The font size of the "Dropped By" text and enemy names can be configured via the "DroppedBy Text Size" plugin parameter (as a percentage of the default font size).
 *   This feature supports items, weapons, and armors as materials.
 *   Compatible with existing save files as it does not modify any save data.
 * - Added <NoDroppedBy> tag for enemies: If an enemy has the <NoDroppedBy> tag in its note box, it will be excluded from the "Dropped By" list, even if it has a drop entry for the material.
 * - Added <NoDroppedBy> tag for Items, Armors, and Weapons: If an Item, Armor, or Weapon has the <NoDroppedBy> tag in its note box, the "Dropped By" section for that material will be excluded entirely in the crafting UI, even if enemies drop it.
 * - Added "Sold In" feature: Displays "[SoldIn Text]: [City1, City2]" below each required material in the crafting UI.
 *   The plugin parses the Item/Armor/Weapon database directly for the <Sold In: Location1, Location2> note tag and lists them comma-separated exactly like the Dropped By feature.
 *
 * Note Tag Format (in Enemies, Items, Armors, or Weapons database):
 * <Item X: [Number]%>
 * <Armor Y: [Number]%>
 * <Weapon Z: [Number]%>
 * <NoDroppedBy> (For Enemies: Excludes the enemy from being listed in "Dropped By"; For Items/Armors/Weapons: Excludes the "Dropped By" section entirely for that material)
 * <Sold In: City1, City2> (For Items/Armors/Weapons: Explicitly adds a 'Sold In' comma-separated text list under the material)
 *
 * Examples:
 * - <Item 353: 50%> (Enemy drops Item ID 353, e.g., "[DroppedBy Text]: Slime")
 * - <Armor 10: 30%> (Enemy drops Armor ID 10)
 * - <Weapon 5: 20%> (Enemy drops Weapon ID 5)
 * - <NoDroppedBy> (In Enemy: Enemy will not be listed in "Dropped By"; In Item/Armor/Weapon: "Dropped By" section will not be shown for that material)
 * - <Sold In: Valenka City, Serenus Town> (Adds cities where the material is sold)
 *
 * Debugging:
 * - If the "Dropped By" line does not appear, check the console (F8 in RPG Maker MV) for debug messages to verify if the note tags are being detected in the Enemies database.
 * - If the "Dropped By" line appears for a material that has <NoDroppedBy> in its note box, check the console for debug messages to ensure the tag is being detected.
 */
...
(function() {
    ...
    var reqmaterial = parameters['Required Material Text'] || 'Required materials:';
	var matreturn = parameters['Returned Material Text'] || 'Returned materials:';
	...
	var droppedByText = parameters['DroppedBy Text'] || 'Dropped By:';
	var soldInText = parameters['SoldIn Text'] || 'Sold In:';
	var droppedByTextSize = Number(parameters['DroppedBy Text Size'] || 75);
    ...
    var item = recipe._result._item;
    width = width || 312;
    
    if (item) {
        var iconBoxWidth = Window_Base._iconWidth + 4;
        
        // Reset text color for item icon
        this.resetTextColor();
        this.drawIcon(item.iconIndex, x + 2, y + 2);
        
        // Check for rarity and apply color
        ...
        this.resetTextColor(); // Reset color after drawing
    }
};
	...
	Window_RecipeDetail.prototype.contentsHeight = function() {
		...
		if(this._recipe) {
			...
			// Base height for materials
			var materialLines = this._recipe._materials.length;
			// Adjust for bonus materials (not shown in crafting mode)
			for (var i = 0; i < this._recipe._materials.length; i++) {
				if (!this._reverse && this._recipe._materials[i]._bonus) {
					materialLines--;
				}
			}
			height += this.lineHeight() * materialLines;
			// Add extra height for "Dropped By" & "Sold In" lines if not in reverse mode
			if (!this._reverse) {
				var extraLines = 0;
				for (var i = 0; i < this._recipe._materials.length; i++) {
					var item = this._recipe._materials[i];
					if (item._bonus) continue;
					var dropInfo = this.getDropInfo(item);
					if (dropInfo.length > 0) {
						extraLines++; // At least one line for "Dropped By"
						// Check if we need an extra line for remaining enemies
						var originalFontSize = this.contents.fontSize;
						this.contents.fontSize = Math.floor(originalFontSize * (droppedByTextSize / 100));
						var enemies = dropInfo.map(info => info.enemyName);
						var firstLineEnemies = [];
						var remainingEnemies = [];
						var currentWidth = 48 + (Window_Base._iconWidth * 0.50) + 4 + this.textWidth(droppedByText) + 4;
						var quantityX = this.contents.width - 72; // Position of the quantity text (e.g., "0/5")
						var maxEnemiesFirstLine = 0;
						for (var j = 0; j < enemies.length; j++) {
							var enemyText = j === 0 ? enemies[j] : ", " + enemies[j];
							var enemyWidth = this.textWidth(enemyText);
							if (currentWidth + enemyWidth <= quantityX && maxEnemiesFirstLine < 4) {
								firstLineEnemies.push(enemies[j]);
								currentWidth += enemyWidth;
								maxEnemiesFirstLine++;
							} else {
								remainingEnemies.push(enemies[j]);
							}
						}
						if (remainingEnemies.length > 0) {
							extraLines++; // Add an extra line for remaining enemies
						}
						this.contents.fontSize = originalFontSize;
					}

					var soldInInfo = this.getSoldInInfo(item);
					if (soldInInfo.length > 0) {
						extraLines++; // At least one line for "Sold In"
						var originalFontSize = this.contents.fontSize;
						this.contents.fontSize = Math.floor(originalFontSize * (droppedByTextSize / 100));
						var locations = soldInInfo;
						var firstLineLocs = [];
						var remainingLocs = [];
						var currentWidth = 48 + (Window_Base._iconWidth * 0.50) + 4 + this.textWidth(soldInText) + 4;
						var quantityX = this.contents.width - 72;
						var maxLocsFirstLine = 0;
						for (var j = 0; j < locations.length; j++) {
							var locText = j === 0 ? locations[j] : ", " + locations[j];
							var locWidth = this.textWidth(locText);
							if (currentWidth + locWidth <= quantityX && maxLocsFirstLine < 4) {
								firstLineLocs.push(locations[j]);
								currentWidth += locWidth;
								maxLocsFirstLine++;
							} else {
								remainingLocs.push(locations[j]);
							}
						}
				...
				height += extraLines * this.lineHeight();
			}
		}
		return Math.max(this.height - this.standardPadding(), height);
	}
	Window_RecipeDetail.prototype.getDropInfo = function(material) {
		var dropInfo = [];
		var type = material._item === $dataItems[material._item.id] ? "Item" :
		           material._item === $dataWeapons[material._item.id] ? "Weapon" :
		           material._item === $dataArmors[material._item.id] ? "Armor" : null;
		if (!type) return dropInfo;

		// Check for <NoDroppedBy> tag in the material's note box (Item, Armor, or Weapon)
		var materialNote = material._item.note;
		if (materialNote.match(/<NoDroppedBy>/i)) {
			return dropInfo; // Return empty array to skip "Dropped By" display
		}

		var itemId = material._item.id;
		// Scan the Enemies database
		for (var enemyId = 1; enemyId < $dataEnemies.length; enemyId++) {
			var enemy = $dataEnemies[enemyId];
			if (!enemy) continue;
			var note = enemy.note;
			// Check for <NoDroppedBy> tag in enemy note box
			if (note.match(/<NoDroppedBy>/i)) {
				continue; // Skip this enemy if it has the <NoDroppedBy> tag
			}
			var regex = new RegExp(`<${type}\\s+${itemId}\\s*:\\s*(\\d+)%>`, 'i');
			var match = note.match(regex);
			if (match) {
				dropInfo.push({ enemyId: enemyId, enemyName: enemy.name });
			}
		}
		return dropInfo;
	}
	Window_RecipeDetail.prototype.getSoldInInfo = function(material) {
		if (!material || !material._item || !material._item.note) return [];
		var note = material._item.note;
		var match = note.match(/<Sold In:\s*(.+?)>/i);
		if (match) {
	...
	Window_RecipeDetail.prototype.drawMaterials = function() {
		...
		for(var i = 0;i < this._recipe._materials.length;i++) {
			...
			// Check for enemies that drop this material & cities where it's sold (only in crafting mode)
			if (!this._reverse) {
				var dropInfo = this.getDropInfo(item);
				if (dropInfo.length > 0) {
					// Move to the next line to draw "Dropped By"
					yy += this.contents.fontSize + this.textPadding();
					// Reset paint opacity to ensure visibility
					this.changePaintOpacity(true);
					// Set font size for "Dropped By" text based on plugin parameter
					var originalFontSize = this.contents.fontSize;
					this.contents.fontSize = Math.floor(originalFontSize * (droppedByTextSize / 100));
					// Draw the icon (icon index 92) at 48 pixels indent, scaled to 50%
					var iconScale = 0.50; // Scale the icon to 50% of its original size
					var iconWidth = Window_Base._iconWidth * iconScale;
					var iconHeight = Window_Base._iconHeight * iconScale;
					...
					// Adjust Y position to vertically center the icon with the text
					var iconY = yy + (this.lineHeight() - iconHeight) / 2;
					this.contents.blt(iconBitmap, sx, sy, pw, ph, 48, iconY, iconWidth, iconHeight);
					// Draw "Dropped By:" text after the icon
					this.changeTextColor(this.systemColor());
					var droppedByLabel = droppedByText; // Use the configurable parameter
					this.drawText(droppedByLabel, 48 + iconWidth + 4, yy, this.contents.width);
					// Split enemy names into first line (3–4 enemies) and remaining
					var enemies = dropInfo.map(info => info.enemyName);
					var firstLineEnemies = [];
					var remainingEnemies = [];
					var currentWidth = 48 + iconWidth + 4 + this.textWidth(droppedByLabel) + 4;
					var quantityX = this.contents.width - 72; // Position of the quantity text (e.g., "0/5")
					var maxEnemiesFirstLine = 0;
					for (var j = 0; j < enemies.length; j++) {
						var enemyText = j === 0 ? enemies[j] : ", " + enemies[j];
					...
					// Draw the first line of enemies
					this.changeTextColor(this.normalColor());
					var firstLineText = firstLineEnemies.join(", ");
					var textX = 48 + iconWidth + 4 + this.textWidth(droppedByLabel) + 4;
					this.drawText(firstLineText, textX, yy, this.contents.width - textX);
					// Draw remaining enemies on the next line if necessary
					if (remainingEnemies.length > 0) {
						yy += this.lineHeight();
						var remainingText = remainingEnemies.join(", ");
						this.drawText(remainingText, 48 + iconWidth + 4, yy, this.contents.width - (48 + iconWidth + 4));
					}
					// Restore original font size
					this.contents.fontSize = originalFontSize;
				}

				// Check for Sold In locations
				var soldInInfo = this.getSoldInInfo(item);
				if (soldInInfo.length > 0) {
					yy += this.contents.fontSize + this.textPadding();
					this.changePaintOpacity(true);
					...
					var iconHeight = Window_Base._iconHeight * iconScale;
					var iconBitmap = ImageManager.loadSystem('IconSet');
					var iconIndex = 208; // Default map/shop/coin icon index for Sold In
					var pw = Window_Base._iconWidth;
					var ph = Window_Base._iconHeight;
					var sx = (iconIndex % 16) * pw;
					var sy = Math.floor(iconIndex / 16) * ph;
					...
					this.changeTextColor(this.systemColor());
					var soldInLabel = soldInText;
					this.drawText(soldInLabel, 48 + iconWidth + 4, yy, this.contents.width);
					
					var locations = soldInInfo;
					var firstLineLocs = [];
					var remainingLocs = [];
					var currentWidth = 48 + iconWidth + 4 + this.textWidth(soldInLabel) + 4;
					var quantityX = this.contents.width - 72;
					var maxLocsFirstLine = 0;
					for (var j = 0; j < locations.length; j++) {
						var locText = j === 0 ? locations[j] : ", " + locations[j];
					...
					this.changeTextColor(this.normalColor());
					var firstLineText = firstLineLocs.join(", ");
					var textX = 48 + iconWidth + 4 + this.textWidth(soldInLabel) + 4;
					this.drawText(firstLineText, textX, yy, this.contents.width - textX);
					
					if (remainingLocs.length > 0) {
						yy += this.lineHeight();
						var remainingText = remainingLocs.join(", ");
						this.drawText(remainingText, 48 + iconWidth + 4, yy, this.contents.width - (48 + iconWidth + 4));
					}
					this.contents.fontSize = originalFontSize;
				}
	...
	Window_RecipeDetail.prototype.setTextFail = function(reverse) {
		this.contents.clear();
