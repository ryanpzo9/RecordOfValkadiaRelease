...
/*:
 * @plugindesc (v1.1) Modifies the file save and load scene.
 * @author Moghunter
 *
 * @param Font Size
 ...
 *
 * @param Text Color
 * @desc Text color definition (System color ID).
 * @default 21
 *
 * @param Word Playtime
 * @desc Definition of the word for "Playtime".
 * @default Playtime
 *
 * @param Word Location
 * @desc Definition of the word for "Location".
 * @default Location
 *
 * @param Display Gold
 * @desc Display the gold/money.
 * @default true
 *
 * @param Actor X-Axis
 * @desc X-axis position of the character image.
 * @default 100
 *
 * @param Actor Y-Axis
 * @desc Y-axis position of the character image.
 * @default 0
 *
 * @param Help X-Axis
 * @desc X-axis position of the help window.
 * @default 0
 *
 * @param Help Y-Axis
 * @desc Y-axis position of the help window.
 * @default 30
 *
 ...
 * @desc Definition of the vertical space between parameters.
 * @default 28
 * * @param Parback X-Axis
 * @desc X-Axis position of the parameter background image.
 * @default 0
 * * @param Parback Y-Axis
 * @desc Y-Axis position of the parameter background image.
 * @default 320
 * * @param Cursor X-Axis
 * @desc X-Axis position of the cursor.
 * @default 342
 * * @param Cursor Y-Axis
 * @desc Y-Axis position of the cursor.
 * @default 0
 * * @param Cursor File Space
 * @desc Definition of the horizontal space between save files.
 * @default 159
 *
 * @help  
 * =============================================================================
 ...
 * =============================================================================
 * Modifies the scene for saving and loading files.
 *
 * =============================================================================
 * USAGE
 * =============================================================================
 * System images must be saved in the folder:
 *
 * /img/menus/file/
 *
 * =============================================================================
 *
 * And picture images in the folder:
 *
 * /img/pictures/
 *
 * Name the character files as follows:
 *
 * Actor_ + ID.png
 *
 * Example:
 ...
 * ** History **
 * =============================================================================
 * v1.1 - Added Mouse Scroll (Wheel) support.
...
Scene_File.prototype.create = function() {
	...
	if (this._listWindow._index < 0 || this._listWindow._index > 5 ) {this._listWindow._index = 0};
...
Scene_File.prototype.nextIndex = function(value) {
	 ...
	 if (this._listWindow._index < 0) {this._listWindow._index = 5};
	 if (this._listWindow._index > 5) {this._listWindow._index = 0};
}; 
...
Scene_File.prototype.updateCommand = function() {
    ...
    else if (TouchInput.wheelY > 0) { this.nextIndex(1) }
    else if (TouchInput.wheelY < 0) { this.nextIndex(-1) }
    // ----------------------------
...
Scene_File.prototype.refreshActorPicture = function() {
	  ...
	  this._ActorPicture.bitmap = ImageManager.loadPicture("ActorSave_" + String(this._actorIndex));
...
Window_FileData.prototype.drawData = function() {
	  ...
	  this._actorID = actor ? actor[10] : 0; 

	  this.changeTextColor(this.textColor(Moghunter.scFile_TextColor));
	  this.drawText(String(Moghunter.scFile_PlayTimeWord), x, y + s * 10, 120, 'left');
	  this.drawText(String(Moghunter.scFile_LocationWord), x, y + s * 11, 120, 'left');
	  
      this.drawText(TextManager.levelA, w1, y, 120, 'left'); // Level
	  this.drawText(TextManager.param(0), x, y + s * 1, 180, 'left'); // HP
	  this.drawText(TextManager.param(1), x, y + s * 2, 180, 'left'); // MP
	  this.drawText(TextManager.param(2), x, y + s * 3, 180, 'left'); // Atk
	  this.drawText(TextManager.param(3), x, y + s * 4, 180, 'left'); // Def
	  this.drawText(TextManager.param(4), x, y + s * 5, 180, 'left'); // Mat
	  this.drawText(TextManager.param(5), x, y + s * 6, 180, 'left'); // Mdf
	  this.drawText(TextManager.param(6), x, y + s * 7, 180, 'left'); // Agi
	  this.drawText(TextManager.param(7), x, y + s * 8, 180, 'left'); // Luk
	  if (this._gold) {this.drawText(TextManager.currencyUnit, x, y + s * 9, 120, 'left')}; // Gold
	  this.resetTextColor();	  
      
      if (this.data().playtime) {
          this.drawText(this.data().playtime, w1 - 40, y + s * 10, 120, 'right'); 
      }
      
      if (this.data().location) {
          this.drawText(this.data().location, w1 - 95, y + s * 11, 175, 'right');
      }
	  ...
	  if (this._gold && this.data().gold) {
          this.drawText(this.data().gold, w1 - 40, y + s * 9, 120, 'right')
      }; // Gold	
};
