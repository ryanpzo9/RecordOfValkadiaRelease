 ...
 *
 * @param Blocked Items
 * @desc List of item IDs or ranges to block in the menu after Retry Battle (e.g., 14, 15, 16-18).
 * @default
 *
 * @help
 *
 ...
 *
 *
 *   InitializeGameOverCore
 *
 * Initializes the plugin's data for compatibility with old save files.
 *
 *
 * ==============================================================================
 *  Force Retry Battle
 ...
 *
 * ==============================================================================
 *  Item Blocking with Note Tags
 * ==============================================================================
 *
 * You can block specific items in the retry menu by adding the following note tag
 * to the item's note box in the database:
 *
 *   <Retry Item Blocked>
 *
 * This will prevent the item from being used in the menu after selecting "Retry Battle",
 * in addition to any items listed in the Blocked Items parameter.
 *
 *
 * ==============================================================================
 *  End of Help File
...
// Parse Blocked Items with support for ranges (e.g., "14, 15, 16-18")
_.blockedItems = (function() {
    const input = String(params['Blocked Items']).split(',').map(str => str.trim());
    let result = [];
    input.forEach(entry => {
        if (entry.includes('-')) {
            const [start, end] = entry.split('-').map(num => parseInt(num));
...
// Function to check if an item has the <Retry Item Blocked> note tag
_.isItemBlockedByNote = function(item) {
    return item && item.note.includes('<Retry Item Blocked>');
};
...
_.retryBattle = function() {
    if ($gameTemp._setUpRetry) {
        ...
        // Use the current $gameParty with updated battle members (set in Scene_Menu.popScene)
...
Game_Temp.prototype.initialize = function() {
    ...
    // Initialize plugin data for old saves
    this._setUpRetry = this._setUpRetry || null;
    this._pendingRetry = this._pendingRetry || false; // Flag for retry after menu
};
...
Game_System.prototype.initialize = function() {
    ...
    // Initialize plugin data for old saves
    this._GOCGameOver = this._GOCGameOver || _.image;
    this._GOCCommonEvents = this._GOCCommonEvents || [_.start, _.middle, _.end];
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    ...
    if(com === 'setgameoverimage') {
        ...
        _.retryBattle();
    } else if(com === 'initializegameovercore') {
        // Manually initialize plugin data for old saves
        $gameTemp._setUpRetry = null;
        $gameTemp._pendingRetry = false;
        $gameSystem._GOCGameOver = _.image;
        $gameSystem._GOCCommonEvents = [_.start, _.middle, _.end];
    }
};

//-----------------------------------------------------------------------------
// Game_Actor (Block items during retry menu)
//-----------------------------------------------------------------------------

_.Game_Actor_meetsItemConditions = Game_Actor.prototype.meetsItemConditions;
Game_Actor.prototype.meetsItemConditions = function(item) {
    const canUse = _.Game_Actor_meetsItemConditions.apply(this, arguments);
    if ($gameTemp._pendingRetry && (_.blockedItems.includes(item.id) || _.isItemBlockedByNote(item))) {
        return false; // Block item if in retry mode and ID is in blocked list or has note tag
    }
    return canUse;
};
...
Scene_Gameover.prototype.retryCommand = function() {
    if ($gameTemp._setUpRetry) {
        // Resurrect and restore all actors before opening the menu
        $gameParty.members().forEach(function(actor) {
            actor.revive();      // Remove death state
            actor.recoverAll();  // Fully restore HP, MP, and states
        });
        // Set the retry flag
        $gameTemp._pendingRetry = true;
        // Open the menu (Yanfly's party menu will override this if active)
...
_.Scene_Menu_popScene = Scene_Menu.prototype.popScene;
Scene_Menu.prototype.popScene = function() {
    _.Scene_Menu_popScene.apply(this, arguments);
    if ($gameTemp._pendingRetry) {
        // Update $gameTemp._setUpRetry[3] with the current $gameParty state after menu changes
        if (Imported.YEP_PartySystem) {
            // Ensure all current party members (including reserves) are revived/restored
            $gameParty.members().forEach(function(actor) {
                actor.revive();
                actor.recoverAll();
            });
            $gameTemp._setUpRetry[3] = JsonEx.makeDeepCopy($gameParty);
        }
        // Load all actor images to ensure faces and sprites are ready
        if (Imported.YEP_PartySystem) {
...
//-----------------------------------------------------------------------------
// Window_ItemList (Gray out blocked items)
//-----------------------------------------------------------------------------

_.Window_ItemList_isEnabled = Window_ItemList.prototype.isEnabled;
Window_ItemList.prototype.isEnabled = function(item) {
    const enabled = _.Window_ItemList_isEnabled.apply(this, arguments);
    if ($gameTemp._pendingRetry && item && (_.blockedItems.includes(item.id) || _.isItemBlockedByNote(item))) {
        return false; // Disable item if in retry mode and ID is blocked or has note tag
    }
    return enabled;
};
