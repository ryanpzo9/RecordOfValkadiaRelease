 ...
 *
 * @param Ougi Sound
 * @desc Sound play when activating Ougi
 * @default Flash2
 *
 * @param Ougi Volume
 * @desc Sound volume play when activating Ougi
 * @default 100
 *
 * @param Ougi Pitch
 * @desc Sound pitch play when activating Ougi
 * @default 100
 *
 * @param Ougi Pan
 * @desc Sound pan play when activating Ougi
 * @default 100
 *
 * @help  
 * =============================================================================
 ...
 *
 * ActorAnim_ID.png
 * ActorAnim_ID + SUFIX.png
 *
 * EnemyAnim_ID.png
 * EnemyAnim_ID + SUFIX.png 
 *
 * Exemplo
 *
 * Actor_2.png
...
　　var Moghunter = Moghunter || {}; 
	...
	Moghunter.sound = String(Moghunter.parameters['Ougi Sound']);
	Moghunter.volume = Number(Moghunter.parameters['Ougi Volume']);
	Moghunter.pitch = Number(Moghunter.parameters['Ougi Pitch']);
	Moghunter.pan = Number(Moghunter.parameters['Ougi Pan']);
...
Scene_Battle.prototype.createOugi = function() {
    this._spriteOugi = [];
	var osound = Moghunter.sound;
	var ovolume = Moghunter.volume;
	var opitch = Moghunter.pitch;
	var opan = Moghunter.pan;
	var mySe = {  name: osound,  volume: ovolume,  pitch: opitch,  pan: opan};
	AudioManager.playSe(mySe);
	if ($gameTemp._ougiData[1].isActor()) {
	   this._spriteOugi[0] =  new Sprite(ImageManager.loadPicture("ActorAnim_" + $gameTemp._ougiData[1]._actorId + $gameTemp._ougiData[4]));
       this._spriteOugi[1] =  new Sprite(ImageManager.loadPicture("ActorAnim_" + $gameTemp._ougiData[1]._actorId));
    } else {
	   this._spriteOugi[0] =  new Sprite(ImageManager.loadPicture("EnemyAnim_" + $gameTemp._ougiData[1]._enemyId + $gameTemp._ougiData[4]));
       this._spriteOugi[1] =  new Sprite(ImageManager.loadPicture("EnemyAnim_" + $gameTemp._ougiData[1]._enemyId));
	};
